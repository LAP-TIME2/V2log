...
### A. 요구사항 → 수용기준(AC) → 금지사항 먼저 확정
수정 요청 시 반드시 3가지를 먼저 정의:
- **요구사항(What)**: 무엇을 바꿔야 하는가
- **수용기준(AC)**: "완료" 판정 기준 (수치/조건 명시)
- **금지사항(Don't)**: 건드리면 안 되는 것

예시:
- AC: "모든 막대에서 라벨이 중앙 정렬", "간격 최소 8px", "기존 터치 동작 유지"

### B. 2-스텝 수렴 방식
한 번에 완벽하게 하려 하지 말고:
1. **1차 패치**: 원인 가설 + 최소 변경으로 문제 해결
2. **2차 패치**: 리팩토링/코드 정리

### C. 변경 범위(Blast Radius) 통제
- "관련 파일 1~2개만 수정"
- "API/모델/DB는 건드리지 않음"
- "UI만 수정" 등 범위 고정

### D. 2회 실패 시 중단 → 원인분해 모드
1~2회 시도 후 AC 불만족 시:
1. 왜 실패했는지 분해 (좌표계/레이아웃/라이브러리 제약 등)
2. 정석 패턴 확인 (공식 문서/예제)
3. 대안 2~3개 비교
4. 가장 안전한 안으로 재시도
