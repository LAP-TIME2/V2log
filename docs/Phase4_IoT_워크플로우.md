...
- [ ] MAX7219 + 7-세그먼트 LED 숫자 표시
- [ ] 세트 LED 10개 (WS2812B) 개별 제어
- [ ] +/- 버튼 입력 → 무게 조정
- [ ] 진동 모터 PWM 제어
- [ ] NFC 태그 읽기 → 기구 ID 인식
...
- OTA DFU 구현 경험
- MAX7219/WS2812B 드라이버 경험

**업무 범위** (8개):
1. IMU 센서 SPI 드라이버 개발 (FIFO 읽기, 캘리브레이션)
2. 횟수 카운팅 알고리즘 구현 (Peak Detection + Velocity Gate + Confirmation System)
3. BLE GATT 서비스 구현 (GATT 스펙 문서 제공)
4. 7-세그먼트 LED 제어 (MAX7219, SPI)
5. 세트 LED 제어 (WS2812B 10개)
...
**업무 범위** (6개):
1. 회로도 설계 (nRF52840 모듈 + IMU + MAX7219 + WS2812B + TP4056 + NFC)
...
## 6. 미결정 사항 (MEMORY.md 연동)

> 📌 = 미결정, ✅ = 확정

| # | 항목 | 상태 | 옵션 | 추천 | 데드라인 |
|---|------|------|------|------|---------|
| 1 | 센서 모델 | 📌 | ICM-45686 vs LSM6DSV | 프로토타입 ICM → 양산 LSM 검토 | Phase A 1주차 |
| 2 | 총 세트 수 입력 위치 | 📌 | 앱 vs 컨트롤러 | 앱에서 설정 → BLE 전송 | Phase A 1주차 |
| 3 | 횟수 동기화 방식 | 📌 | 실시간 BLE vs 세트 완료 후 배치 | 배치 (전력 절약) | Phase A 2주차 |
| 4 | 7-seg 드라이버 IC | 📌 | MAX7219 vs TM1637 | MAX7219 (SPI 통일) | Phase A 1주차 |
| 5 | 세트 LED 방식 | 📌 | 개별 LED + 74HC595 vs WS2812B | WS2812B (GPIO 1개) | Phase A 1주차 |
