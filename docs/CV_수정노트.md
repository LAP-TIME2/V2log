...
## v2B-010: 헬스장 실기기 테스트 버그 4건 수정 (2026-02-16)

### 증상 4가지

| # | 증상 | 심각도 |
|---|------|--------|
| 1 | 세트 완료 후 즉시 Stage 1 복귀 → 원판 안 바꿨는데 재감지 | 중 |
| 2 | "바벨을 카메라에 보여주세요" 텍스트가 화면 중앙 가림 | 중 |
| 3 | Stage 1→2 전환 시 높이 280→200 점프 + Stage 2 화각 부족 | 높 |
| 4 | 배경 랙 원판 오감지 (5kg×2=30kg인데 60~80kg) | 치명 |

### 원인 1: 세트 완료 → 무조건 재감지
`didUpdateWidget`에서 `completedSets` 변경 시 `_detectedWeight = null` + Stage 1 복귀 → 원판을 안 바꿨어도 처음부터 재감지. 사용자 입장에서는 매 세트마다 "바벨을 보여주세요"가 뜸.

**그러니까**: 같은 무게로 계속 하는데 매 세트마다 "무게가 뭐야?" 라고 다시 물어보는 거예요.

### 원인 2: Stage 1 UI가 중앙을 가림
`Center > Column`에 큰 아이콘 + 큰 텍스트 → 바벨을 가져다 대면 텍스트에 가려서 원판이 안 보임.

### 원인 3: 높이 불일치
Stage 1=280px, Stage 2=200px → 전환 시 레이아웃 점프. Stage 2에서 벤치프레스 이완 시 팔이 프레임 밖으로 나감.

### 원인 4: 전체 프레임 처리 + ROI 필터 없음
카메라에 사용자 바벨 + 배경 랙 원판이 동시에 보임. `_parseOutput()`에 공간 필터 없음 → 배경 원판도 전부 감지.

**그러니까**: AI가 화면 전체에서 원판을 찾으니까, 멀리 있는 다른 랙의 원판까지 다 세서 무게가 60~80kg으로 뜬 거예요.

### 해결

**문제 1+3-B: 30초 모니터링 모드**
- 첫 세트: 기존 방식 (YOLO 안정화 → 즉시 확정)
- 2세트+: 이전 무게를 기억하고 30초간 원판 변경 감시
  - 변화 없으면 자동 확정 → Stage 2
  - 2.5kg+ 변화 감지 → 리셋 + 새 무게 재감지
  - [확정] 버튼으로 즉시 넘어갈 수도 있음
- `_detectedWeight`를 null로 안 지움 → 모니터링 가능

**문제 2: 하단 compact 바 + ROI 가이드**
- `Center > Column` 삭제 → 하단에 compact 상태 바
- 화면 중앙에는 ROI 가이드 박스만 (밖 반투명, 안 투명)
- `_RoiOverlayPainter` CustomPainter: `Path.combine(difference)`로 ROI 밖만 어둡게

**문제 3-A: 높이 280 통일**
- Stage 1, 2, 로딩 placeholder 모두 280px
- 전환 시 레이아웃 점프 제거 + 넓은 시야

**문제 4: 3단계 배경 필터**
- 4-A: UI ROI 가이드 (사용자에게 "이 안에 맞추세요" 유도)
- 4-B: 서비스 ROI 공간 필터 (xCenter/yCenter가 0.15~0.85, 0.20~0.80 밖이면 무시)
- 4-C: `_minBboxArea` 0.005 → 0.01 (2m+ 배경 원판 0.3~0.8% → 필터됨)

### 배운 점
1. **세트 간 무게 변경은 "예외"** — 대부분 같은 무게로 계속함. 매번 재감지는 UX 악화
2. **카메라 UI 중앙은 비워야** — 사용자가 원판을 대는 곳인데 텍스트로 가리면 안 됨
3. **ROI는 UI + 서비스 양쪽 모두 필요** — UI 가이드만 있으면 배경 처리, 서비스 필터만 있으면 사용자 혼란
